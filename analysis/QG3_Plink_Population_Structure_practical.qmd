...
The file “[YRI_CEU_ASW_MEX_NAM.bed](https://github.com/joellembatchou/SISG2026_Association_Mapping/tree/master/data)” is a binary file in PLINK BED format with accompanying BIM and FAM files.
...
**File with ancestry labels assignment for each sample**: [Population_Sample_Info.txt](https://raw.githubusercontent.com/joellembatchou/SISG2026_Association_Mapping/master/data/Population_Sample_Info.txt) 
...
HGDP_dir <- "/Users/joelle.mbatchou/SISG/2026/SISG2026_Association_Mapping/data/" 
```
...
4. Interpret the first two PCs, what ancestries are they reflecting?
    * **Think about**: Do PCs represent individual SNPs or overall patterns of genetic variation? Which ancestries form distinct clusters?
...
5. Read in the eigenvalues and make a scree plot corresponding for these first 10 PCs. Estimate the proportion of variance explained by the first two PCs.
    * **Hint**: To calculate proportion of variance, sum the first 2 eigenvalues and divide by the sum of all eigenvalues
```{r eval=FALSE}
...
6. Now redo Question 2 above using the [`bigsnpr` R package](https://privefl.github.io/bigsnpr/reference/index.html) specifying a $r^2$ threshold of 0.2 (i.e. LD pruning) as well as a minimum minor allele count (MAC) of 20.
```{r eval = FALSE}
...
* Check the SNP loadings for the first 5 PCs.
    * **Interpret**: What do SNP loadings tell you? Which SNPs drive PC1 vs PC2?
...
8. Make a barplot of the proportional Native American/European ancestry estimates based on your output of question 7.
    * **Interpret**: Do MXL individuals show admixture (mixed ancestry)? What proportion is Native American vs European on average?
```{r, eval = FALSE}
...
1. **PCA captures ancestry**: PC1/PC2 represent continental-level ancestry differences, not individual SNPs
2. **Population stratification = confounding**: If cases/controls differ in ancestry, you see spurious associations
3. **LD pruning improves signal**: Correlated SNPs don't add information; pruning reduces noise in PCA
4. **PCA is exploratory**: Use it to identify population structure, then adjust for PCs in downstream analysis
5. **Admixture detection**: Some samples show mixed ancestry from multiple populations
